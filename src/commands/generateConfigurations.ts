...
export async function generateConfigurationsObject(
  traits: any,
): Promise<boolean> {
  let generateSuccessful: boolean = true;
  let configs = {
  ...
  try {
    for(let trait in traits) {
      configs.order.push(trait);

      const attributes = traits[trait];
      const randoms = generateRandoms(Object.keys(attributes).length - 1);
      const tmp = {};
