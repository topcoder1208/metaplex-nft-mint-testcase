...
import { arweaveUploadBuffer } from '../helpers/upload/arweave';
...
export async function upload(
  ...
  for (let i = 0; i < SIZE; i++) {
    ...
    if (i % 50 === 0) {
      console.info(`Processing file: ${i}`);
    } else {
...
export async function uploadObjects(
  ...
  Object.keys(files).forEach(f => {
    if (!seen[f.replace(EXTENSION_PNG, '').split('/').pop()]) {
      seen[f.replace(EXTENSION_PNG, '').split('/').pop()] = true;
      newFiles.push(f);
      fileIds[f] = f.replace(EXTENSION_PNG, '').split('/').pop();
      fileBuffers[f] = files[f];
    }
  });
  existingInCache.forEach(f => {
    if (!seen[f]) {
  ...
  for (let i = 0; i < SIZE; i++) {
    ...
    let link = cacheContent?.items?.[index]?.link;
    if (!link || !cacheContent.program.uuid) {
      const manifestContent = JSON.stringify(metadatas[fileIds[imageName]])
                                  .replace(imageName, 'image.png')
                                  .replace(imageName, 'image.png');
      const manifest = JSON.parse(manifestContent);
      ...
      if (!link) {
        try {
          if (storage === 'arweave') {
            link = await arweaveUploadBuffer(
              walletKeyPair,
              anchorProgram,
              env,
              fileBuffers[imageName],
              manifestBuffer,
              manifest,
              index,
            );
