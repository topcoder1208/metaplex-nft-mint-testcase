...
export async function createMetadataObjects(
  numberOfImages: number,
  config: any,
): Promise<any[]> {
  let _metadataObjects = [];
  let numberOfObjectsCreated: number = 0;
  const randomizedSets = [];
  ...
  while (numberOfObjectsCreated < parseInt(numberOfImages, 10)) {
    const randomizedSet = generateRandomSet(breakdown);
    ...
    if (!_.some(randomizedSets, randomizedSet)) {
      ...
      _metadataObjects.push(metadata);
  ...
  // map through after because IDs would make sets unique
  const _randomSetWithIds = randomizedSets.map((item, index) => ({
    id: index + 1,
    ...item,
  }));

  return {_metadataObjects, _randomSetWithIds};
}
