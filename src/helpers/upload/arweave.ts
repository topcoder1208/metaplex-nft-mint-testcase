...
async function upload(data: FormData, manifest, index) {
  console.debug(`trying to upload ${index}.png: ${manifest.name}`);
  return await (
...
function estimateManifestSize(filenames: string[]) {
  ...
  console.debug('Estimated manifest size:', data.length);
  return data.length;
}
...
export async function arweaveUpload(
  ...
  console.debug(`solana transaction (${env}) for arweave payment:`, tx);
...
export async function arweaveUploadBuffer(
  walletKeyPair,
  anchorProgram,
  env,
  imageBuffer,
  ...
  console.log(`lamport cost to store: ${storageCost}`);
